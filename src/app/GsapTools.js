...
class GsapTools extends PureComponent {
  ...
  onKeyDown = e => {
    ...
    if (e.keyCode === 32) {
      ...
      // E char
      this.handleExpand();
    } else if (e.keyCode === 82 && !(e.metaKey || e.ctrlKey)) {
      // R char
      this.range.clear();
  ...
  checkUIPosition = (state, prevState) => {
    ...
    if (!state) {
      ...
      const x =
        cl < bounds ? tx - (cl - bounds) : cr > innerWidth - bounds ? 0 : tx; // eslint-disable-line
  ...
  initUI = () => {
    ...
    const isVisible =
      storageIsVisible === null ? this.props.isVisible : storageIsVisible;
    ...
    this.master = new TimelineMax({
      ...
      onComplete: () => {
        if (this.state.isLoop && !this.isChanging) {
          ...
          this.outTime &&
          this.master.totalProgress() === this.outTime
        ) {
          this.setState({ playIcon: true });
        }
      }
  ...
  initInOut = ({ inTime, outTime, marker } = {}) => {
    ...
    this.inOutMaster = this.master
      .tweenFromTo(this.inTime, this.outTime, {
        onStart: () => {
